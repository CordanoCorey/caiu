...
export class CalendarEventFormComponent extends FormComponent
  implements OnInit {
  ...
  constructor(
    @Optional() public dialogRef?: MatDialogRef<CalendarEventFormComponent>,
  ...
  get calendarEvent(): CalendarEvent {
    return this.data['calendarEvent'] || this.calendarEvent;
  }
  ...
  get calendarEventFormTemplate(): TemplateRef<any> {
    return (
      this.data['calendarEventFormTemplate'] || this._calendarEventFormTemplate
    );
  }
      ...
      eventName: this.form.value.eventTypeId
        ? build(
            ...
            this.calendarEventTypes.find(
              x => x.id === this.form.value.eventTypeId
            )
          ).name
  ...
  ngOnInit() {
    this.setValue(this.calendarEvent);
  }
