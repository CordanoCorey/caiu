...
import { SchedulerCalendarEvent } from '../scheduler.model';
...
export class EventCreatorDialogComponent implements OnDestroy, OnInit {
  @Control(SchedulerCalendarEvent) form: FormGroup;
  ...
  get eventsToday(): any[] {
    return this.data.events.filter(
      x =>
        x.monthOf === this.data.dayInfo.month &&
        x.dayOf === this.data.dayInfo.date &&
        x.yearOf === this.data.dayInfo.year
    );
  }
  ...
  get valueOut(): SchedulerCalendarEvent {
    return build(SchedulerCalendarEvent, this.form.value, {
      description: this.form.value.eventName,
      eventId: this.eventId,
      eventName: this.form.value.eventTypeId
        ? build(
            LookupValue,
            this.eventTypes.find(x => x.id === this.form.value.eventTypeId)
          ).name
  ...
  ngOnInit() {
    ...
    if (
      (this.data.editing && this.eventsToday.length >= 2) ||
      !this.data.editing
    ) {
      if (this.data.id !== undefined) {
        const selectedEvent = this.eventsToday.filter(
          x => x.eventId === this.data.id
        );
  ...
  eventSelected(event) {
    if (event.value === undefined) {
      ...
      this.form
        .get('startTime')
        .get('hour')
        .setValue(event.startTime.hour);
      this.form
        .get('startTime')
        .get('minute')
        .setValue(event.startTime.minute);
      this.form
        .get('startTime')
        .get('timePeriod')
        .setValue(event.startTime.timePeriod);
      this.form
        .get('endTime')
        .get('hour')
        .setValue(event.endTime.hour);
      this.form
        .get('endTime')
        .get('minute')
        .setValue(event.endTime.minute);
      this.form
        .get('endTime')
        .get('timePeriod')
        .setValue(event.endTime.timePeriod);
    } else {
      ...
      this.form
        .get('startTime')
        .get('hour')
        .setValue(event.value.startTime.hour);
      this.form
        .get('startTime')
        .get('minute')
        .setValue(event.value.startTime.minute);
      this.form
        .get('startTime')
        .get('timePeriod')
        .setValue(event.value.startTime.timePeriod);
      this.form
        .get('endTime')
        .get('hour')
        .setValue(event.value.endTime.hour);
      this.form
        .get('endTime')
        .get('minute')
        .setValue(event.value.endTime.minute);
      this.form
        .get('endTime')
        .get('timePeriod')
        .setValue(event.value.endTime.timePeriod);
    }
  }
}
